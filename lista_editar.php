...
include 'conexao.php';
	...
	$sql = mysql_query("SELECT * FROM tb_produtos WHERE gtin OR nome LIKE '{$_POST['busca_gtin']}%' ORDER BY gtin ASC");
	}
	else {
...
<body link=black alink='green' vlink='black' Onload="inicio()">
	<div class="container">
		...
		<b><h1><font color="#602d8f"> Digite o GTIN do medicamento: </b></h1></font>
			<input type="text" size="35" name="busca_gtin" id="txt_busca">
			<input type="submit" value="FILTRAR" id="btn_filtrar">
			<input type="submit" onClick="document.form1.action='cadastro_produto.html'" value="VOLTAR" id="btn_voltar">
		</form>
		<br><br><br>
...
<?php	
	...
	while($linha = mysql_fetch_assoc($sql)) {
		...
		<th><a href="editar_produto.php?edit=<?php echo $linha['gtin']; ?>&nome=<?php echo $linha['nome']; ?>&instrucoes=<?php echo $linha['instrucoes']; ?>&dosagem=<?php echo $linha['dosagem']; ?>&preco=<?php echo $linha['preco']; ?>&quantidade=<?php echo $linha['quantidade']; ?>"><img src='imagens/write.png' height='50' width='50'></a></th>
		<tr>
		<br>
<?php }	?>
</table>
